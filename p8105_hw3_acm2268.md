...
```
    ...
    ## # A tibble: 39 x 4
    ## # Groups:   aisle [39]
    ##    aisle                    department       n `fct_reorder(aisle, n)` 
    ##    <fct>                    <chr>        <int> <fct>                   
    ##  1 baby food formula        babies       13198 baby food formula       
    ##  2 baking ingredients       pantry       13088 baking ingredients      
    ##  3 bread                    bakery       23635 bread                   
    ##  4 butter                   dairy eggs   10575 butter                  
    ##  5 candy chocolate          snacks       11453 candy chocolate         
    ##  6 canned jarred vegetables canned goods 12679 canned jarred vegetables
    ##  7 canned meals beans       canned goods 11774 canned meals beans      
    ##  8 cereal                   breakfast    16201 cereal                  
    ##  9 chips pretzels           snacks       31269 chips pretzels          
    ## 10 crackers                 snacks       19592 crackers                
    ## # ... with 29 more rows

``` r
ggplot(n_items_df, aes(x = aisle, y = n)) + 
  geom_bar(stat = "identity", fill = "blue") +
  theme(axis.text.x = element_text(family = "Arial", angle = 45)) +
...
```
    ...
    ## 4 dog food care              Organix Grain Free Chicken & Ve~    14            1
    ## 5 dog food care              Organix Chicken & Brown Rice Re~    13            2
...
ggplot(ny_df, aes(x = data_value, fill = response)) + 
  ...
  facet_grid(. ~ year) +
  labs(x = "Data Values", y = "Density", title = "Distribution of Data Values by Response Type among Locations in NY State: 2006 vs. 2010")
```
